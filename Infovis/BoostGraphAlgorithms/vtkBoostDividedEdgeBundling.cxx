...
{
  vtkVector2f xrange(VTK_FLOAT_MAX, VTK_FLOAT_MIN);
  vtkVector2f yrange(VTK_FLOAT_MAX, VTK_FLOAT_MIN);
  vtkVector2f zrange(VTK_FLOAT_MAX, VTK_FLOAT_MIN);
  for (vtkIdType i = 0; i < this->Graph->GetNumberOfVertices(); ++i)
    {
    vtkVector3f p = this->Nodes[i];
    xrange[0] = std::min(xrange[0], p[0]);
    xrange[1] = std::max(xrange[1], p[0]);
    yrange[0] = std::min(yrange[0], p[1]);
    yrange[1] = std::max(yrange[1], p[1]);
    zrange[0] = std::min(zrange[0], p[2]);
    zrange[1] = std::max(zrange[1], p[2]);
    }
  ...
  for (vtkIdType i = 0; i < this->Graph->GetNumberOfVertices(); ++i)
    ...
    this->Nodes[i] = vtkVector3f(
      (p[0] - xrange[0])/extent * 1000.0f,
      (p[1] - yrange[0])/extent * 1000.0f,
      (p[2] - zrange[0])/extent * 1000.0f);
    }
}
...
void vtkBundlingMetadata::CalculateNodeDistances()
{
  ...
  for (vtkIdType u = 0; u < numVerts; ++u)
    ...
    for (vtkIdType v = 0; v < numVerts; ++v)
      {
      std::cout << "u: " << u << ", v: " << v << ", d:" << this->NodeDistances[u][v] << std::endl;
      }
    }
}
...
void vtkBundlingMetadata::SimulateEdgeStep()
{
  ...
  for (vtkIdType e1 = 0; e1 < numEdges; ++e1)
    ...
    for (int m1 = 0; m1 < this->MeshCount; ++m1)
      ...
      //std::cerr << "position " << e1 << ": " << position << endl;
      vtkVector3f velocity = this->EdgeMeshVelocities[e1][m1];
      //std::cerr << "velocity " << e1 << ": " << velocity << endl;
      vtkVector3f acceleration = this->EdgeMeshAccelerations[e1][m1];
      //std::cerr << "acceleration " << e1 << ": " << acceleration << endl;
      ...
      //std::cerr << "prevPosition " << e1 << ": " << prevPosition << endl;
      vtkVector3f prevDirection = prevPosition - position;
      //std::cerr << "prevDirection " << e1 << ": " << prevDirection << endl;
      float prevDist = prevDirection.Norm();
      //std::cerr << "prevDist " << e1 << ": " << prevDist << endl;
      float prevForce = this->EdgeSpringConstant / 1000.0f * (this->MeshCount - 1) * prevDist * weight1;
      //std::cerr << "prevForce " << e1 << ": " << prevForce << endl;
      prevDirection.Normalize();
      //std::cerr << "prevDirection normalized " << e1 << ": " << prevDirection << endl;
      acceleration = acceleration + prevForce * prevDirection;
      //std::cerr << "accelerationA " << e1 << ": " << acceleration << endl;
      //std::cerr << "prevForce " << e1 << ": " << prevForce << endl;
      //std::cerr << "prevDirection " << e1 << ": " << prevDirection << endl;
      //std::cerr << "prevPosition " << e1 << ": " << prevPosition << endl;
      ...
      //std::cerr << "accelerationB " << e1 << ": " << acceleration << endl;

      // Coulomb force
      float normalizedEdgeCoulombConstant = this->EdgeCoulombConstant / sqrt(numEdges);
      //float normalizedEdgeCoulombConstant = this->EdgeCoulombConstant;
      ...
      for (vtkIdType e2 = 0; e2 < numEdges; ++e2)
        ...
        if (dot >= 0.0f)
          ...
          //std::cerr << "position2 " << e1 << ": " << position2 << endl;
          }
        ...
        if (distance > 0.0f)
          ...
          //std::cerr << "accelerationC " << e1 << ": " << acceleration << endl;
          }
      }
  ...
  vtkInformationVector *outputVector)
{
  ...
  vtkNew<vtkFloatArray> edgeCompat;
  edgeCompat->SetName("compatibility");
  edgeCompat->SetNumberOfTuples(g->GetNumberOfEdges());
  output->GetEdgeData()->AddArray(edgeCompat.GetPointer());
  for (vtkIdType e = 0; e < g->GetNumberOfEdges(); ++e)
    {
    edgeCompat->SetValue(e, meta->EdgeCompatibilities[0][e]);
    for (int m = 1; m < meta->MeshCount-1; ++m)
      {
      vtkVector3f edgePoint = meta->EdgeMesh[e][m];
      //std::cerr << "edgePoint " << e << "," << m << ": " << edgePoint << std::endl;
